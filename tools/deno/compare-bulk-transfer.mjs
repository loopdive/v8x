...
const [out, scalar, bulk, scalarCore, bulkCore, provider] = process.argv.slice(2);
if (!provider) throw Error("usage: compare-bulk-transfer.mjs OUT SCALAR_EXE BULK_EXE SCALAR_CORE BULK_CORE PROVIDER");
...
for (let round = 0; round < 5; round++) {
  for (const name of round % 2 ? ["bulk", "scalar"] : ["scalar", "bulk"]) {
    ...
    rows.push(row);
    writeFileSync(out + "/results.json", JSON.stringify({inputs:{scalar,bulk,scalarCore,bulkCore,provider},rows},null,2));
