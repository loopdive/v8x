...
const rows = [];
const controlCollector = process.env.DENO_AB_CONTROL_COLLECTOR ?? "drc";
const candidateCollector = process.env.DENO_AB_CANDIDATE_COLLECTOR ?? "drc";
for (let round = 0; round < 5; round++) {
  for (const name of round % 2 ? ["bulk", "scalar"] : ["scalar", "bulk"]) {
    ...
    env.V8X_JS2WASM_GC_COLLECTOR = name === "bulk" ? candidateCollector : controlCollector;
    ...
    rows.push(row);
    writeFileSync(out + "/results.json", JSON.stringify({inputs:{scalar,bulk,scalarCore,bulkCore,provider,candidateProvider,controlCollector,candidateCollector},rows},null,2));
